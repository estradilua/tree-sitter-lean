...
const commands = {
  ...
  cmd_hash: $ => seq(/#[\pL_!]+/, optional($.term)),
...
export default {
  ...
  struct_explicit_binder: $ => seq('(', $._o, repeat1($.ident), struct_binder_binders($),
    optional($.type_spec), optional(seq($.defeq, $.term)), ')', $._c),
  struct_implicit_binder: $ => seq('{', repeat1($.ident), struct_binder_binders($), $.type_spec, '}'),
  struct_inst_binder: $ => seq('[', repeat1($.ident), struct_binder_binders($), $.type_spec, ']'),
  struct_simple_binder: $ => seq($.ident, optDeclSig($), optional(seq($.defeq, $.term))),
  struct_fields: $ => many1Indent($, seq(declModifiers($), choice($.struct_explicit_binder,
